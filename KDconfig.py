...
def WxPusher(title, message, token):
    
    if token.get('APPTOKEN_token') == '':
        print('WxPusher 推送未配置, 推送失败')
        return False
    APPTOKEN = token.get('APPTOKEN_token')
    UIDS = token.get('UIDS_token')
    print(APPTOKEN,UIDS)
...
sendList = [gocq, pushplus, push, ding, qywx, WxPusher] # 函数名
sendTokenList = ['gocq', 'pushplusToken', 'push+Token', 'dingToken', 'qywxToken', 'WxPusherToken'] # 配置文件中的相关要素
sendMes = ['gocq', 'pushplus', 'push+', 'ding', 'qywx', 'WxPusher'] # 未配置对的提示
# =============== 判断函数 和 发送入口 ======================
